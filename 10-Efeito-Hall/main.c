...
#define ELEMENTOS_FILA_Hall 1

static QueueHandle_t LCD_Show_queue; 
static QueueHandle_t SERVO_queue; 
static QueueHandle_t Hall_Freq_Hz_queue; 
...
static void task6_hall(void *args __attribute ((unused))){
	// Input Pull-Up -> ODR = 1
	GPIOA->CRH	|= 1<<28; 
	GPIOA->ODR	|=	1<<15; 
	...
	u8 leitura_atual =0;
	u8 leitura_anterior =0;


#define AMORTRAS 10

 	while(1){
		leitura_atual = GPIOA->IDR & (1<<15);
		...
		if (pulsos == AMORTRAS){
			periodo = pulsos/contador ; // Período T 
			frequencia = 1/periodo;	// Freq = 1/T

			contador = 0 ; 
			periodo = 0;

			if( uxQueueMessagesWaiting( Hall_Freq_Hz_queue ) <= ELEMENTOS_FILA_Hall ){
				xQueueSend(Hall_Freq_Hz_queue, &frequencia, pdMS_TO_TICKS(10));
			}
...
int main(void) {
	...
	Hall_Freq_Hz_queue = xQueueCreate(ELEMENTOS_FILA_Hall, sizeof(float));
	...
	xTaskCreate(task6_hall, "Freq_Hall", 100 , NULL, 3 , NULL);
